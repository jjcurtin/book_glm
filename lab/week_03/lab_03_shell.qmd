...
We are *buying* one piece of information (the mean) to estimate one parameter ($P = 1$).
...
Because $p = .002$, which is < .05, the additional precision was *worth* the addition of the parameter. The mean of the sample is a better predictor than 0.  Alternatively, we can say that it is unlikely ($p = .002$) that we would have observed this much of a reduction in error from estimating $b_0$ (using the sample mean) if the population mean were actually 0.
...
`tidy()` from the broom package will provide you with the t-statistic and p-value corresponding to each parameter in your model.
```{r}
m_mean |> 

```

To calculate $t$ by hand, we need the parameter estimate and its standard error from the augmented model. We also need the value of the population paramenter if the null hypothesis is true (0 in this instance).  Next, we can obtain the p-value for $t$ using `pt()`. Note that you need to double the number you get if you wanted two-tailed p-value.
