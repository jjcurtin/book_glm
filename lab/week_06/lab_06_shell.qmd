...
This dataframe has row names, whereas a tibble would not. Row names may be problematic as they require different syntax compared to variables in columns. Let's convert row names to a column. Then, convert the dataframe to a tibble so that we can retain these job descriptions in our data file.
...
Use it on our data. But it doesn't make sense to use on character class columns. Let's exclude the job column. 
```{r}
#| message: false
...
Stars gives you the correlation magnitudes (r) with stars denoting p-value significance! This is likely the most useful.
```{r}
...
**We can also see this from a model comparison perspective.**
```{r}
...
Now let's calculate model $R^2$ to compare the full model to the mean only model. $R^2$ should be used when we are comparing models that differ by more than one parameter. 
```{r}

```

Or glance using the full model. 
```{r}
