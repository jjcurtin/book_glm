...
*You want to know if there is a relationship between `income` and job `type` (blue versus white collar). Specifically, you hypothesize that blue collar jobs will have a lower income than white collar jobs.*
...
#### d. Fit a linear model in which income is regressed on `type_wc`. What do the estimates for $b_0$ and $b_1$ represent? Provide a conceptual interpretation for each parameter estimate.
...
#### e. Fit a linear model in which income is regressed on `type_c`. What do the estimates for $b_0$ and $b_1$ represent? Provide a conceptual interpretation for each parameter estimate.
...
Define a function called "pre()" that takes the value of "compact" and "augmented" to calculates the pre.
