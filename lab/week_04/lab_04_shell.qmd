...
These are three different ways that missing data can be handled. It's important to know your data well so you can make informed decisions about handling NAs. *Remember that there is no one right answer for how to handle missing data, and there are other ways than the ones we have covered here.*
...
# or you could use view interactively.  View will not render in knitted documents.
# preds |> view()
```
...
ggplot() +
  ...
  labs(x = "Composite Mental Paper Folding Scores", y = "Distance from Fold")
```
...
If `condition` is 1 and 2, this isn't very meaningful!
```{r}
data_dich <- data_dich |> 
  mutate(condition_num = ) |> 
  glimpse()

lm() 
```

If `condition` is dummy-coded (i.e., 0 and 1), $b0_i$ will represent the `inclusion` score for an individual in the 0 condition (based on how we have ordered our factor variable -- 0 would represent the control condition).
```{r}
data_dich <- data_dich |> 
  mutate() |> 
  glimpse()
...
If `condition` is contrast-coded (i.e., centered, coded as -.5 and .5), $b_0$ will represent the `inclusion` score for an individual with a 0 on `condition`, a hypothetical "average" between our two conditions.
```{r}
data_dich <- data_dich |> 
  mutate(condition_c = ) |> 
  glimpse()
...
**For this lab, we will be using contrast coding (our `condition_c` variable).**
