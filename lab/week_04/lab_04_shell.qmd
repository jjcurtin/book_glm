...
```{r}
data <- read_csv(here::here(path_data, "lab_04_data_cont.csv"),
...
data <- data |> 
  rowwise() |> 
  mutate(mpf_sum = sum(c(mpf_01,mpf_02,mpf_03,mpf_04,mpf_05,
                        mpf_06,mpf_07,mpf_08,mpf_09,mpf_10))) |> 
  ungroup() |> 
  glimpse()
```
...
**Notice we got a warning!**     
Remember, we always treat warnings like errors until we determine the source of the warning and whether it would be expected.    

1. Based on these messages where do you think the source of the warning is?       
In the mutate function. We are not directly calling `if_else` or `c_across` so my hypothesis is that the warning is coming from the function `var_score`.     

2. Let's check this hypothesis by only running the `var_score` function. *Note that the warning tells us it won't display the warning again for 8 hours*, so we need to restart R (session --> Restart R), use the "Run all chunks above" arrow on our previous code block, and then we can test the following code:   

```{r}
var_score(data, forward_items = f_items)
```

Okay now we see this function is in fact the source of the warning. If we look back at John's code from above, we can see that he is using `c_across()` and not specifying `cols`. This warning is telling us that is no longer the current way to use the function. This is what *deprecated* in our warning means -- there is a new/better way to use the `c_across()` function (i.e., supply `cols`), but the old version (i.e., not needing to supply `cols`) is still supported/tolerated (for now!).   

3. How might you update this code? You could copy and paste your warning into Chat GPT. We did this and it told us that instead of using `c_across()` and by default letting it select all columns, you could use `c_across(cols = everything()))`. This would solve the warning.    

4. Since we are not going to rewrite John's function right now, we are going to confirm the function is returning an appropriate/expected value (we already did this for you) and then move on to suppress the warning.   
...
data |> 
  ...
  geom_point(aes(x = mpf_sum, y = distance), 
             alpha = .4) +
  geom_abline(intercept = coef(m_mean), color = "blue")
```
