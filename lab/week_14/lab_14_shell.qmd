...
3. 2-by-2 ANOVA
4. Dummy Coding for multi-level categorical variables
...
### 1. What is the magnitude of the simple effect of time spent outside on happiness for participants who read for pleasure?
    ...
    Plug 0.5 into our equation for read_c   
    $10 + 3 * 0.5 + 2 * outside_c + 0.5 * 0.5 * outside_c$    
    $11.5 + 2.25 * outside_c$   
    The simple effect is 2.25   

    (If students ask) We can use our regression equation to find the slope at two levels of outside_c   
    $10 + 3 * 0.5 + 2 * 0 + 0.5 * 0.5 * 0 = 11.5$   
    $10 + 3 * 0.5 + 2 * 1 + 0.5 * 0.5 * 1 = 13.75$   
    13.75 - 11.5 = 2.25
...
### 2. What is the magnitude of the simple effect of time spent outside on happiness for participants who do not read for pleasure?   
    ...
    $10 + 3 * -0.5 + 2 * outside_c + 0.5 * -0.5 * outside_c$   
    $8.5 + 1.75 * outside_c$   
    The simple effect is 1.75   

    (If students ask) We can use our regression equation to find the slope at two levels of outside_c    
    $10 + 3 * -0.5 + 2 * 0 + 0.5 * -0.5 * 0 = 8.5$   
    $10 + 3 * -0.5 + 2 * 1 + 0.5 * -0.5 * 1 = 10.25$    
    10.25 - 8.5 = 1.75    
...
### 3. What is the effect of reading on happiness for participants with an average number of hours spent outside? Is this a simple or main effect?   
    ...
    (If students ask) We can also demonstrate this algebraically:     
    $10 + 3 * 0.5 + 2 * 0 + 0.5 * 0.5 * 0 = 11.5$   
    $10 + 3 * -0.5 + 2 * 0 + 0.5 * -0.5 * 0 = 8.5$    
    11.5 - 8.5 = 3    

    This is the simple effect of reading, but since the number of hours spent outside is a centered continuous predictor, it is conceptually equivalent to the main effect of reading.   

### 4. Describe in words how the effect of reading on happiness varies by the number of hours spent outside.    

    This value is b3. For every one-unit increase in time spent outside, the (simple) effect of reading increase by 0.5 units.     

### 5. How many hours does a person spend outside if they have a score of -3 on the outside_c predictor?   

    Since our average is 10 hours spent outside, participants with a score of -3 spend 7 hours outside per week.   

### 6. What is the predicted happiness for a person who spends 14 hours outside per week and does not read for pleasure?   

    outside_c = 4, read_c = -0.5
    outside_c = 4 because our intercept represents the average amount of time outside, 10 hours (10 + 4 = 14!)
    $10 + 3 * -0.5 + 2 * 4 + 0.5 * -0.5 * 4 = 15.5$   
...
### 7. Change the coding of reading to 0 = does not read and 1 = reads for pleasure and fill in the resulting equation:    
    ...
    b0 = $10 + 3 * -0.5 + 2 * 0 + 0.5 * -0.5 * 0$    
    b1 = the simple effect of reading on time spent outside for participants with an average amount of time spent outside. b1 does not change (b1 = 3).   
    b2 = the simple effect of spending time outside for participants who do not read for pleasure. We found this in a previous question (b2 = 1.75).    
    b3 = the interaction effect which has also not changed (b3 = 0.5).   

    happiness = 8.5 + 3 * read + 1.75 * outside_c + 0.5 * read * outside_c    
...
### 8. How (if at all) will the standard errors for the parameter estimates for the interaction terms of the two models (one with read_c and one with read) differ?  
    ...
    Our parameter estimates and the associated SE will remain unchanged between the two models. The highest-order interaction is not affected by recoding of the variables that constitute it.    
...
### 9. Write the augmented and compact model for the model comparison which tests whether the read_c * outside_c interaction effect is significant.    
    ...
    Compact: $happiness = b0 + b1 * read_c + b2 * outside_c$    
    Augmented: $happiness = b0 + b1 * read_c + b2 * outside_c + b3 * read_c * outside_c$
...
```{r}
d <- read_csv(here::here(path_data, "lab_12_data.csv"),
...
```{r}
d |> skim_without_charts(age, prosociality)

plot(density(d$age))
plot(density(d$prosociality))
```

### 13. Subset the data and get observations in the Collaborate condition only.

```{r}
d_collab <- d |> filter(condition == 1)
```
...
d_collab <- d_collab |> mutate(
  age_c = age - mean(age),
  prosociality_c = prosociality - mean(prosociality)
)
```
...
```{r}
m_1 <- lm(learning ~ age_c*prosociality_c, data = d_collab)
```
...
d |> 
  group_by(cond) |> 
  summarize(mean_support = mean(support))

d |> 
  group_by(political_dichot) |> 
  summarize(mean_support = mean(support))
```
...
TAs, walk through each step of the code to remind students what each line is doing, but note that they have not learned pivot yet so they will not be writing this code themselves. 
...
b0 = prediction when `cond` and `political_dichot` both are 0 = 5.09
b1 = simple effect of `cond` when `political_dichot` is 0 = `r 5.26 - 5.09`
b2 = simple effect of `political_dichot` when `cond` is 0 = `r 4.90 - 5.09`
b3 = difference in simple effects of `cond` across `political_dichot` 
   = `r (6.01 - 4.90) - (5.26 - 5.09)`
...
```{r}
data <- data |> 
  mutate(education_c = education - mean(education, na.rm = TRUE))
```

Create dummy variables using professional as a reference group
```{r}
data <- data |> 
  mutate(blue_d3 = if_else(type == "Blue Collar", 1, 0),
         white_d3 = if_else(type == "White Collar", 1, 0))
```

Now let's fit the model and see what the output is
```{r}
m_1 <- lm(income ~ blue_d3 + white_d3 + education_c, data = data)
tidy(m_1)
```

income = 7639 + (-102) * blue_d3 + (-2788) * white_d3 + 888 * education_c
...
```{r}
lm(income ~ type + education_c, data) |> tidy()
```

**What model comparison will get you the test of the main effect of group? Run that comparison**

compact: income ~ education_c; augmented: income ~ blue_d3 + white_d3 + education_m

```{r}
anova(lm(income ~ education_m, data), m_1)
```
