...
# Unit 2: Sampling Distributions, Parameters, and Parameter Estimates {.unnumbered} 
...
:::{.fragment}
[Administer the Ocean liking scale to all 10,000 inhabitants in the population and calculate the population mean score. Is it 0?  If not, the inhabitants are not neutral on average.]{style="color:blue;"}
:::
...
options(conflicts.policy = "depends.ok")  # <1> 
library(tidyverse)
path_data <- "data_lecture"  # <2> 

data <- read_csv(here::here(path_data, 
                            "02_sampling_distributions_like.csv"), # <3>
                 show_col_types = FALSE) 
```

1. This line of code will tell will tell us when we load two packages with conflicting functions (i.e., they have the same name) by producing an error. There are ways to set custom conflict policies and work around conflicts (e.g., only loading certain functions from a package). You can find more documentation [here](https://jjcurtin.github.io/book_dwvt/conflicts.html).   
...
:::{.fragment}
[Inhabitants of the island are neutral on average on the Ocean Liking Scale; $\mu$ = 0.]{style="color:blue;"}
:::
...
:::{.fragment}
[Excluding issues of measurement of the scale (i.e., reliability), you are 100% confident that the population mean score on this scale is 0 because you calculated the mean using all the scores in the population($\mu$ = 0).]{style="color:blue;"}
:::
...
:::{.fragment}
[You will very rarely have access to all scores in the population. Instead, you have to use inferential statistics to “infer” (estimate) the size of the population parameter using a parameter estimate calculated from a sample of that population.]{style="color:blue;"}
:::
...
:::{.fragment}
[A sample mean of 2.14 is not 0. However, you know that the sample mean will not match the population mean exactly. How likely is it to get a sample mean of 2.14 if the population mean is 0 (think about it!)?]{style="color:blue;"}
:::
...
:::{.fragment}
[The mean of the sample means (i.e., the mean of the sampling distribution) will equal the population mean of raw scores on the dependent measure.  This is important because it indicates that the sample mean is an unbiased estimator of the population mean.]{style="color:blue;"}
:::
...
:::{.fragment}
[No, there was a distribution of means that varied from each other. The mean of the sampling distribution was the population mean but the standard deviation was not zero.]{style="color:blue;"}
:::
...
:::{.fragment}
[The standard deviation of the population raw scores and the sample size.]{style="color:blue;"}
:::
...
:::{.fragment}
[1. Individual differences, and 2. Measurement error (the opposite of reliability)]{style="color:blue;"}
:::
...
:::{.fragment}
[As the variability (e.g., its standard deviation) of the raw scores on the variable increases in the population, the standard error of the mean increases.]{style="color:blue;"}
:::
...
:::{.fragment}
[The standard error of the mean would equal 0 no matter which participants you sampled. They would all have the same scores!]{style="color:blue;"}
:::
...
:::{.fragment}
[As the sample size increases, the standard error of the mean will decrease.]{style="color:blue;"}
:::
...
:::{.fragment}
[If each sample contained only 1 participant, the standard error of the mean would be equal to the standard deviation ($\sigma$) observed for the population raw scores.  This is the upper bound for how much sampling error you can expect (it is biggest when you have the smallest possible sample size)]{style="color:blue;"}
:::
...
:::{.fragment}
[Pretty likely…   But we can do better than that with our probability estimate!]{style="color:blue;"}
:::
...
:::{.fragment}
[Our sample mean from our study. This is our parameter estimate of the population mean for ocean like scores (like_score) scores.]{style="color:blue;"}
...
$\frac{\sigma}{\sqrt{N_{sample}}}$]{style="color:blue;"}
...
:::{.fragment}
[We can use our sample standard deviation ($s$), but $s$ is a negatively biased parameter estimate.  On average, it will underestimate $\sigma$.]{style="color:blue;"}
...
::::{.fragment}
[We account for this underestimation of $\sigma$ and therefore of the standard deviation (standard error) of the sampling distribution by using the $t$ distribution rather than the $z$ distribution to calculate the probability of our parameter estimate if $H_0$ is true.]{style="color:blue;"}

[The $t$ distribution is slightly wider, particularly for small sample sizes to correct for our underestimate of the standard deviation.]{style="color:blue;"}
::::
:::
