...
data <- read_csv(here::here(path_data, "2_sampling_distributions_like.csv"), # <3>
...
3. we use the `here()` function in the `here` package (`here::here()`) to define paths within a function. This approach (vs. `file.path()`) works well when using R Projects. 
