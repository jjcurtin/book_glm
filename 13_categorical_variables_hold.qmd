...
::: {.footer}
We are usind `_d3` in regressor names to indicate they are dummy codes with the third level as reference
:::
...
- But you also think that the type of psychopathology matters (AUD is worse than depression)
...
[A common alternative:]{style="color:blue;"}       
  ...
  mutate(c_1a = case_match(group,
        ...
        c_1b = case_match(group,
         ...
         c_2b = case_match(group,
                           ...
                           "depress" ~ -1,
                           "healthy" ~ 0),
        c_1c = case_match(group,
                           "alcohol" ~ 0.333,
                           "depress" ~ 0.333,
                           "healthy" ~ -.667),
        c_2c = case_match(group,
                           "alcohol" ~ .5,
                           "depress" ~ -.5,
                           "healthy" ~ 0))
```
...
m_c_c <- lm(health ~ c_1c+ c_2c, data = data)
```
...
::: {.callout-important}
# Question
Why are the parameter estimates for `c_1` different and why do we prefer option c?
:::

:::{.fragment}
[In each instance $b_1$ represents change in health for a 1 unit change on the `c_1` regressor. For `c_1c` a 1 unit change moves fully from the alcohol/depress group to the healthy group. In contrast, $b_1$ was 2/3 the size for `c_1a` because 1 unit only moves 2/3 of the way between the two sets in the c_c1 contrast.]{style="color:blue;"}        
...
How would you describe `c_1` (patient groups vs. healthy controls) and `c_2` (patients with alcohol use disorder vs. depression) in terms of Set U and Set V?   And what coefficients would you assign?
:::

:::{.fragment}
$C_1 = (c_1*alcohol + c_2*depress)-(c_3*healthy)$    

[Set U: `alcohol`, `depress`; Set V: `healthy`]{style="color:blue;"}         


$C_2 = (c_1*alcohol)-(c_2*depress)$     

[Set U: `alcohol`; Set V: `depress`; Ignored: `healthy`]{style="color:blue;"}

[Patient vs. healthy = (.333 * alcohol + .333 * depress) - (.667 * healthy)]{style="color:blue;"}
[alcohol vs. depress = (.5 * alcohol) - (.333 * depress) - (0 * healthy)]{style="color:blue;"}
:::

--------------------------------------------------------------------------------

We can set these contrasts up as a matrix in R and then apply them to the group factor
```{r}
