...
And now:  **Direct + Indirect/Spurious = Total**
`r round(coef(m_exam_3)[2], 2)` +  (`r round(coef(m_iq)[2], 2)` * `r round(coef(m_exam_3)[3], 2)`) = `r round(coef(m_exam_2)[2], 2)`
...
Lets return to our running example of BAC effects on FPS.
...
Look at the parameter estimate for `bac` across these two models

-----

:::{.callout-important}
# Question
Why did $b$ for BAC get smaller (less negative) when TA was controlled? Hint: consider the bivariate relationships between all variables.
...
:::{.fragment}
[1. When BAC increases, TA decreases.]{style="color:blue;"}

[2. When TA decreases FPS decreases.  This negative *indirect/spurious effect* contributes to the total effect of BAC (negative $b$ for BAC in 1 predictor model).]{style="color:blue;"}

[3. The partial effect of BAC on FPS, if TA was not allowed to decrease (i.e., is controlled or held constant), is less negative than when it is allowed to naturally decrease as BAC increases.]{style="color:blue;"}
:::
...
:::{.callout-important}
# Question
In what situation would $b_j$ for a focal predictor not change when you added an additional predictor (covariate)? </span> 
:::

:::{.fragment}
[1. If the new predictor was completely uncorrelated (orthogonal) with the focal predictor in those sample data, there would be no change in the parameter estimate when you added this new predictor.]{style="color:blue;"}
