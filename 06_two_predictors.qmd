...
:::{.callout-note}
# Note 
Formula for the standard error for $b_0$ is different than for $b_j$.  For the two parameter model $SE_{b0}$ is 

$\sqrt {\frac{SSE}{N-P}}*\sqrt{\frac{1}{N}+\frac{X^2}{(N-1)s_x^2}}$


(we need matrix notation for $P > 2$)
:::
...
The two (and $k$) predictor models are simple generalizations of the models you have already learned
...
## Visualizing Total and Direct/Unique Effects with Venn Diagrams
...
- The numbers represent variability8 (in SS) in $Y$ attributed to the $X$s.  
...
- All of these variabilities can be quantified as differences in SSE or $var(\hat{Y_i})$ between various models.  
- This will help us think more carefully about the model comparisons we are using to test the effects of our predictors.  

\

Lets fit all the relevant models here
...
- Remember that the SSE of the mean-only model is the numerator of the variance of `exam`: $SSE_{mean-only} = \Sigma{(Y_i - \hat{Y})^2}$ where $\hat{Y} = \bar{Y}$ 
...
The diagram above represents the explained and unexplained variability in `exam` from the `study_hours` one predictor model.
...
:::{.callout-important}
# Question
How can we get the variability in `exam` explained by `study_hours` in this one predictor model using SSEs from two models (hint, think about the relevant model comparison)?
:::

:::{.fragment}
[We use a model comparison between the mean-only model and the one predictor study hours model to test the parameter estimate for study hours. The reduction in error across these two models is the variability that is "explained" (predicted) by study hours. ]{style="color:blue;"}
...
We can do the same thing for the `iq` one predictor model
...
:::{.callout-important}
# Question
How do we get the variability in `exam` explained/predicted by `iq` in this one predictor model.
:::

:::{.fragment}
[A model comparison between the mean only model and the one predictor iq model. Error is reduced in the `iq` model because `iq` explains variability that was previously unexplained in the mean-only model.]{style="color:blue;"}

- $SSE_{m\_mo} - SSE_{m\_iq}$
...
Review $\Delta R^2$ for `intervention_group` across two models.  Do they make sense to you?
...
- how to indicate the location of $b_0$, $b_1$, and $b_2$ in figures regardless of how $X$s are coded.
...
- How to understand $R^2$, $\Delta R^2$, and $\eta_p^2$ in terms of venn diagrams of variance.
- How to define compact and augmented models for model comparisons to test hypotheses about $R^2$ and parameter estimates. 
- How to define $R^2$, $\Delta R^2$, and $\eta_p^2$ in terms of SSE from compact, augmented models and mean-only models.
- Text, table, and figure descriptions of results.  