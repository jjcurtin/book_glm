...
my_skim <- skimr::skim_with(numeric = list(hist = NULL, p25 = NULL, p100 = NULL))
```

```{rlout .callout-info}
m_2 <- lm(1 + fps ~ bac, data = data)
...
What could we change about the sampling distribution that would make this $b_1$ be less probable given $H_0$ so that we reject the Null?</span> 
:::
...
$R^2_j$ = variance in $X_j$ accounted for by all other regressors in the model (i.e., how redundant is $X_{a_j}$ in model?). Literally, predict $X_j$ as $Y$ with all other regressors as $X$s.  

\

**Note: Formula for the standard error for $b_0$ is different.** For the two parameter model it is (need matrix notation for $P > 2$):  
...
:::{.callout-info}
# Question
What can we do analytically to decrease SSE (increase model $R^2$)?
:::

:::{.fragment}
[Include another predictor (*covariate*) in the model that accounts for additional variance in $Y$ (reduces SSE).]{style="color:blue;"}

[Ideally, this covariate should be orthogonal (uncorrelated) with the other predictor (BAC).]{style="color:blue;"}
...
s <- skimr::skim_without_charts(data)
```
...
<span style="color: blue;">We use $b_1$ (`r round(coef(m_3)[2], 1)`) to test our hypothesis about the population effect of BAC ($\beta_1$).</span>
...
$\text{SSE}_c = `r round(sum(residuals(lm(fps ~ ta, data=data))^2),1)`$  
...
$\text{SSE}_a = `r round(sum(residuals(lm(fps ~ bac + ta, data=data))^2),1)`$

$P_c = 3$  

$F(P_a - P_c, N - P_a) = \frac{(\text{SSE}_c = \text{SSE}_a)/(P_a-P_c)}{\text{SSE}_a/(N-P_a)}$   
$F (1, 93) = `r round((sum(residuals(lm(fps ~ ta, data=data))^2) - sum(residuals(lm(fps ~ bac + ta, data=data))^2))/(sum(residuals(lm(fps ~ bac + ta, data=data))^2)/93), 2)`, p = `r round(pt(-2.04, 93, lower.tail = TRUE)*2, 4)`$
...
$\text{SSE} =$ `r round(sum(residuals(m_2)^2),1)`
...
$\text{SSE} =$ `r round(sum(residuals(m_3)^2),1)`
...
Compact Model: $\hat{\text{FPS}}= `r round(coef(lm(fps~1, data = data)),1)` + 0*\text{BAC}$   
$\text{SSE}_c = `r round(sum(residuals(lm(fps~1, data = data))^2),1)`$  
$P_c$ = 1  

Augmented Model: $\hat{\text{FPS}}= `r round(coef(m_2)[1],1)` + `r round(coef(m_2)[2],1)` *\text{BAC}$   
...
Compact Model: $\hat{\text{FPS}}= `r round(coef(lm(fps~ta, data = data))[1],1)` + 0*\text{BAC} + `r round(coef(lm(fps~ta, data = data))[2],1)` * \text{TA}$   
$\text{SSE}_c = `r round(sum(residuals(lm(fps~ta, data = data))^2),1)`$  
$P_c$ = 2  

Augmented Model: $\hat{\text{FPS}}= `r round(coef(m_3)[1],1)` + `r round(coef(m_3)[2],1)` *\text{BAC} + `r round(coef(m_3)[3],1)` *\text{TA}$   
$\text{SSE}_a = `r round(sum(residuals(m_3)^2),1)`$  
$P_a$ = 3  
...
<span style="color: blue;">$b_1= `r round(coef(m_exam_1)[2],2)`$</span>

```{r}
broom::tidy(m_exam_2)
```

<span style="color: blue;">$b_1= `r round(coef(m_exam_2)[2],2)`$</span>

```{r}
broom::tidy(lm(iq ~ study_hours_c, data = data_exam))
```

<span style="color: blue;">$b_1= `r round(coef(lm(iq ~ study_hours_c, data = data_exam))[2],2)`$</span>
...
`r round(coef(m_exam_2)[2],2)` +  (`r round(coef(lm(iq ~ study_hours_c, data = data_exam))[2],2)` * `r round(coef(m_exam_2)[3],2)`) = `r round(coef(m_exam_1)[2],2)`
...
**$\text{SSR} = `r round(sum(residuals(lm(fps~1, data = data))^2) -  sum(residuals(m_2)^2),1)`$**
...
**$\text{SSR} = `r round(sum(residuals(lm(fps~ta, data = data))^2) -  sum(residuals(m_3)^2),1)`$**
...
$\text{SSE}_{\text{mean-only}} = `r  round(sum(residuals(lm(fps~1, data = data))^2),1)`$

**Augmented Model: $\hat{\text{FPS}}_i=\beta_0+\beta_1*\text{BAC}_i+ \beta_2*\text{TA}_i$**   

$\text{SSE}_a = `r  round(sum(residuals(m_3)^2),1)`$

$R^2 = \frac{`r  round(sum(residuals(lm(fps~1, data = data))^2),1)` - `r  round(sum(residuals(m_3)^2),1)`}{`r  round(sum(residuals(lm(fps~1, data = data))^2),1)`} = `r round((sum(residuals(lm(fps~1, data = data))^2) - sum(residuals(m_3)^2))/ sum(residuals(lm(fps~1, data = data))^2),4)`$
...
<span style="color: blue;">$\frac{\text{SSE}_c - \text{SSE}_a}{\text{SSE}_c} = \frac{`r  round(sum(residuals(lm(fps~ta, data = data))^2),1)` - `r  round(sum(residuals(m_3)^2),1)`}{`r  round(sum(residuals(lm(fps~ta, data = data))^2),1)`} = `r round((sum(residuals(lm(fps~ta, data = data))^2)-sum(residuals(m_3)^2)) /sum(residuals(lm(fps~ta, data = data))^2), 4)`$</span>
...
$R^2 = `r round(broom::glance(lm(fps~ta, data = data))$r.squared, 4)`$
...
$R^2 = `r round(broom::glance(m_3)$r.squared, 4)`$


$\Delta R^2= `r round(broom::glance(m_3)$r.squared, 4)` -  `r round(broom::glance(lm(fps~ta, data = data))$r.squared, 4)` = `r round(broom::glance(m_3)$r.squared, 4) - round(broom::glance(lm(fps~ta, data = data))$r.squared, 4)`$
...
<span style="color: blue;">$\frac{\text{SSE}_c - \text{SSE}_a}{\text{SSE}_{\text{mean-only}}}=\frac{`r round(sum(residuals(lm(fps~ta, data = data))^2),1)` - `r  round(sum(residuals(m_3)^2),1)`}{`r  round(sum(residuals(lm(fps~1, data = data))^2),1)`} = `r round((sum(residuals(lm(fps~ta, data = data))^2) - sum(residuals(m_3)^2))/(sum(residuals(lm(fps~1, data = data))^2)), 4)`$</span>
