...
And now:  **Total effect = Direct + Indirect/Spurious**
...
- You can either `pull()` or `select()` `r.squared` to get the numeric value or a tibble with the value, respectively
...
- Total SSE in $Y$
  ...
  - SSE in mean-only model 
```{r}
(sse(m_mo) - sse(m_full)) + sse(m_full)
