...
2. Learn models with multiple regressors in later units:      
  ...
  - Multiple predictors and regressors   
    ...
    *Interactive:*    
	      - Factorial ANOVA   
	      - Attitude Treatment Interactions  
       	- General Interactive Models    
...
labels <- tibble(means = c(round(mean(subset(data, bg_2 == 0)$fps), 2), 
                 ...
                 vars = c("bg_1", "bg_2"))
plot_d +
  annotate("text", x = -.2, y = labels$means[1], label = labels$means[1], 
           size = 5, color = "blue") +
...
<span style="color: blue;">Compact model:  $\hat{\text{FPS}}=\beta_0+0*\text{bg_2}_i$</span>  
$P_c = 1$  
$\text{SSE}_c = `r sum((data$fps - mean(data$fps))^2) ` $  


<span style="color: blue;">Augmented model:  $\hat{\text{FPS}}=\beta_0+\beta_1*\text{bg_2}_i$</span>   
$P_a=2$  
$\text{SEE}_a = `r sum((data$fps - predict(m_d))^2)`$
...
Compact model:  $\hat{\text{FPS}}=\beta_0+0*\text{bg_2}_i$  
$P_c = 1$  
$\text{SSE}_c = `r sum((data$fps - mean(data$fps))^2) ` $  


Augmented model:  $\hat{\text{FPS}}=\beta_0+\beta_1*\text{bg_2}_i$   
$P_a=2$  
$\text{SEE}_a = `r sum((data$fps - predict(m_d))^2)`$  
...
<span style="color: blue;">$\frac{\text{SSE}_c - \text{SSE}_a}{\text{SSE}_c}= \frac{`r sum((data$fps - mean(data$fps))^2)` - `r sum((data$fps - predict(m_d))^2)`}{sum((data$fps - mean(data$fps))^2)} = `r round((sum((data$fps - mean(data$fps))^2)-sum((data$fps - predict(m_d))^2))/sum((data$fps - mean(data$fps))^2), 4)`$</span>
...
<span style="color: blue;">Compact model:  $\hat{\text{FPS}}=0+\beta_1*\text{bg_2}_i$</span>  
$P_c = 1$  
$\text{SSE}_c = `r sum((data$fps - predict(lm(fps~bg_2 -1, data = data)))^2) ` $  
...
Compact model:  $\hat{\text{FPS}}=0+\beta_1*\text{bg_2}_i$  
$P_c = 1$  
$\text{SSE}_c = `r sum((data$fps - predict(lm(fps~bg_2 -1, data = data)))^2) ` $  
...
<span style="color: blue;">$\frac{\text{SSE}_c - \text{SSE}_a}{\text{SSE}_c}= \frac{`r sum((data$fps - predict(lm(fps~bg_2 -1, data = data)))^2) ` - `r sum((data$fps - predict(m_d))^2)`}{sum((data$fps - predict(lm(fps~bg_2 -1, data = data)))^2) } = `r round((sum((data$fps - predict(lm(fps~bg_2 -1, data = data)))^2) -sum((data$fps - predict(m_d))^2))/sum((data$fps - predict(lm(fps~bg_2 -1, data = data)))^2), 4)`$</span>
...
$\frac{\text{SSE}_\text{mean only}-\text{SSE}_a}{\text{SSE}_\text{mean only}}= \frac{`r sum((data$fps - mean(data$fps))^2) ` - `r sum((data$fps - predict(m_d))^2)`}{`r sum((data$fps - mean(data$fps))^2) ` } = `r round((sum((data$fps - mean(data$fps))^2) - sum((data$fps - predict(m_d))^2))/sum((data$fps - mean(data$fps))^2),4)` $ 
