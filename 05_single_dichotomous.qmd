...
- The significance of the statistical test isn't affected by which group you assign to be the reference.
...
[That said, some predictor variables start as dichotomous variables so we need to be able to accomodate those variables in our models]{style="color:blue;"}
...
[However, a one unit change in $X$ moves from the placebo group (coded 0) to alcohol group (coded 1).  Therefore $b_1$ is now the change in $\hat{Y}$ between placebo and alcohol groups. This is simply the difference in the mean of the two groups!]{style="color:blue;"}
:::
...
::: {.callout-important}
# Question
Why does $R^2$ equal PRE for $\beta_1$ for this two parameter model?
:::

:::{.fragment}
[In the two parameter model, only $X_1$ and its associated parameter estimate, $b_1$ contribute to making different predicted values for each participant.  Therefore reduction in SSE due to $\beta_1$ is the same as the increase in variance explained by the full model.  Only $\beta_1$ * $X_1$ explain variance in predictions.]{style="color:blue;"}
:::
...
:::{.fragment}
[It is the unweighted mean. An unweighted mean is simply the mean of the group means]{style="color:blue;"}
...
Here is the mean of the means
```{r}
(30.5 + 37.1) / 2
```

This can be thought of as giving equal weight to the two group means when combining into a single mean.
```{r}
(.5 * 30.5) + (.5 * 37.1)
```
:::
...
The concept of weighted and unweighted means emerges with grouped data.

\

An **unweighted** mean is the mean of the group means, ignoring the $N$ in each group.  It gives equal weight to each group mean regardless of the sample size in each group.
...
Again, here is the mean FPS in the two groups
```{r}
#| code-fold: true
...
A weighted mean would weight those two means by their relative sample sizes
```{r}
(.75 * 30.5) + (.25 * 37.1)
```

And this is equivalent to the mean of the DV when you ignore the groups
```{r}
data |> summarise(mean = mean(fps))
```
...
To get $b_0$ to reflect the weighted mean of the two groups, you simply need to mean center the regressor you calculated for the group variable $X$. 

This is the same as we did with a quantitative variable to make $b_0$ equal the grand mean.  You can mean center either a dummy coded regressor or a regressor that uses centered coefficients]{style="color:blue;"}

Here we do this using the previously calculated dummy coded regressor `bg_d`
```{r}
data <- data |> 
  mutate(bg_mc = bg_d - mean(bg_d))
```

The new codes are still unit weighted but they are no longer centered on 0.
```{r}
data |> slice_sample(n = 8)
```

-----

And here are the parameter estimates for this model.  Notice the new value for $b_0$
```{r}
...
Mean centered coefficients are less commonly used with dichotomous or other categorical variables.  

Typically, you will only see them used where the sample sizes of the groups reflect something about the real world rather than just the researchers decision to regarding group size.  This happens mostly when you are randomly sampling from the population (or some other representive sampling method), rather than using convenience samples or experimental work where you assign participants to groups.

But it is important to understand because it deepens your intuition about how these models work.   
...
- How to code a dichotomous predictor into a regressor using dummy coding (0/1) and centered coefficients (-.5/.5)
- Why you should not dichotomize quantitative predictors (lower power, higher p-values, more error)
- The impact of using other codes for the groups
- How to interpret $b_0$ and $b_1$ in these models
- How to test hypotheses about $b_0$ and $b_1$ using NHST using either the sampling distribution or model comparisons perspectives
- How to calculate PRE ($\eta_p^2$) for $b_0$ and $b_1$
- How to calculate $R^2$ for the full model
- The difference between weighted and unweighted means and how to get each using different coding schemes
- How to visualize these models (both quick/dirty and publication quality)