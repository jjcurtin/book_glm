...
::: {.callout-important}
# Exercise
Quantify effect of bias from an appropriate linear model.
:::

:::{.fragment}
[You can use any linear model.  I prefer one that is raw on quant predictors and dummy coded for dichotomous predictors]{style="color:blue;"}
```{r}
#| code-fold: true
  
lm (guilt ~ black_defend * bias, data = data) |> 
  tidy()
```

- $\text{guilt} = 19.2 + -5.9*\text{black defend} + 0.6*\text{bias} + .4*\text{black defend}*\text{bias}$  


[For White defendants:]{style="color:blue;"}
    
- Bias effect = $0.6 + .4*\text{black defend}$  
- Bias effect = $0.6 + .4*0$  
- Bias effect = $0.6$
...
Defendant race effect = $-5.9 + .4*\text{bias}$  
Defendant race effect = $-5.9 + .4*35$  [NOTE: mean of bias = ~35]
Defendant race effect = $8.5$ (w/rounding error)

[-1 SD:]{style="color:blue;"}

Defendant race effect = $-5.9 + .4*\text{bias}$  
Defendant race effect = $-5.9 + .4*51$  [NOTE: +1 SD above mean of bias = ~51]
Defendant race effect = $1.8$ (w/rounding error)

[+1 SD:]{style="color:blue;"}

Defendant race effect = $-5.9 + .4*\text{bias}$  
Defendant race effect = $-5.9 + .4*19$  [NOTE: -1 SD above mean of bias = ~19]
Defendant race effect = $15.2$ (w/rounding error)
