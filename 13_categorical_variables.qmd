...
Each parameter estimate for the $X$s represents the contrast between the mean of $Y$ in its respective target group (the group that was coded 1 for that $X$) and the **reference** group. This is because each effect represents the effect of that specific $X$ holding constant the other $X$ constant at 0.
...
For example, consider the effect of the first regressor (`alcohol_d3`). Considered by itself, it is the contrast of patients with alcohol use disorder (1) vs. other groups (0). 

However, if you held the second regressor (`depress_d3`) constant at zero you would be examining the effect of patients with alcohol use disorder vs. other groups when it is 0 on `depress_d3` (i.e., the reference group). 

```{r}
tibble_d
```

NOTE: More on separate visual and mathematical ways to think about this later
...
Either way, $R^2$ is the proportion of variance explained in `health` by `group` (all three levels)
...
::: {.callout-important}
# Question
How will the analyses be similar and different?
:::
:::
