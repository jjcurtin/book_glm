...
::: {.callout-important}
# Question 
How have we handled categorical variables in our linear models and what constraint has been true for all of our categorical variables so far?
:::

::: {.fragment}
[We have handled categorical variables by recoding using dummy codes or centered coefficients. This turned categorical/nominal variables into numeric variables.]{style="color:blue;"}

[However, so far, we have only handled categorical variables with 2 levels.]{style="color:blue;"}
:::

--------------------------------------------------------------------------------

These single $X$ variables coded with dummy or contrast systems yielded one comparison of the mean of $Y$ across the two levels of the categorical variable.
...
We will start with generalizing the dummy coding approach to more than 2 levels
...
Each regression coefficient represents the contrast between the mean of $Y$ in its respective target group (the group that was coded 1 for that contrast) and the **reference** group. This is because each effect represents the effect of that specific $X$ holding constant the other $X$ constant at 0.
...
For example, consider the effect of the first regressor (`alcohol_d`). Considered by itself, it is the contrast of patients with alcohol use disorder (1) vs. other groups (0). 

However, if you held the second regressor (`depress_d`) constant at zero you would be examining the effect of patients with alcohol use disorder vs. other groups when it is 0 on `depress_d` (i.e., the control group). 
...
- depress vs. healthy = 6.44 - 7.94 = -1.50j
```{r}
data |> 
  group_by(group) |> 
...
We set up `group` as a factor
```{r}
class(data$group)
```

We set up the levels of the factor earlier with 
...
We can reset the reference level (or even change the contrast type - more on that in a moment) 
...
- This can be quicker/easier
- It does hide some of what is going on (not great for understanding)
- Results are, of course, the same!
```{r}
m <- lm(health ~ group, data = data)
tidy(m)
```

You can use the approach (manually coding $X$s or contrasts on factor) that best suits your needs
...
::: {.callout-important}
# Question 
How are these t-tests of $b_j$ in the linear different from what you would get if you simply ran two separate between groups t-tests?
:::
...
We could do this using `contra.treatment()`
```{r}
...
```{r}
m_d2 <- lm(health ~ group, data = data)
```

--------------------------------------------------------------------------------

Or we could do this by manually calculating two new dummy coded $X$s using depress as the level that is set to zero for both $X$s
...
--------------------------------------------------------------------------------
Remember that these are teh group means
...
:::{.fragment}
[Model $R^2$ is the variance in $Y$ explained by all the regressors in the model. It will only test the main effect of an IV if only regressors for that IV are in the model.]{style="color:blue;"}
:::

--------------------------------------------------------------------------------

How do you get model $R^2$ and its test for our model?

- You can get the value using `glance` as always
```{r}
glance(m_d3)$r.squared
```

To test the combined effect of the two dummy coded regressors you can do a model comparison

-  Augmented model is full with both dummy regressors vs.
-  Compact model is mean only model
```{r}
anova(lm(health ~ 1, data = data), m_d3)
```

Alternatively, if you fit the model with contrasts on `group`, then `anova()` will give you this test if you use only the full model (again, not great for understanding!)
```{r}
anova(m)
```

This is the proportion of variance explained in `health` by `group` (all three levels)

--------------------------------------------------------------------------------

We can also get variance based effect sizes for each contrast
...
We can set up our functions for $\Delta R^2$ 
...
PRE for alcohol vs. healthy (compace model includes only the depress vs. healthy contrast)

```{r}
delta_r2(lm(health ~ depress_d3, data = data), m_d3)
```

PRE for depress vs. healthy
```{r}
