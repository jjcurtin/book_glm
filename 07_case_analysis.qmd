...
library(patchwork)
library(skimr)
library(car, exclude = c("recode", "some")) #<1>
...
1. We are handling conflicts with two functions in the `car` package by excluding them upon loading. As a reminder you can see more about handling conflicts [here](https://jjcurtin.github.io/book_dwvt/conflicts.html).   
...
```{r}
case_analysis <- function(model, type) {
  ...
  if(type == "df_betas") {
    m_1_coefs <- names(as_tibble(dfbetas(m_1)))
    i <- 0
  ...
  if(type == "influence_plot") {
    t_cut_point <- qt(p = .025/nrow(data), 
                       ...
                       lower.tail = FALSE)
    p <- data |> 
      mutate(residuals = rstudent(model),
