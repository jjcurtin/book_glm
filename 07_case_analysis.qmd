...
:::{.callout-tip}
see `?skim_with` and `?sfl` for details on customizing `skim()` 

This function can be sourced from lab_support repository:
`source("https://github.com/jjcurtin/lab_support/blob/main/my_skim.R?raw=true")`
:::

---------------------------------------------------------------------------

Skim the data (excluding `subid`)
```{r}
data |> my_skim(-subid)
```
...
As always, lets be "DRY" by writing functions (and add to your personal function library?)
...
`source("https://github.com/jjcurtin/lab_support/blob/main/case_analysis.R?raw=true")`
:::
...
$\text{SE}_{bj} = \frac{s_y}{s_j} = \frac{\sqrt{(1-R^2_y)}}{\sqrt{(N - k - 1)}}*\frac{1}{\sqrt{(1-R^2_j)}}$
...
plot_leverage <- function(model) {
  tibble(x = hatvalues(model)) |> 
    ggplot(aes(x = x)) +
      ...
      labs(subtitle = "") +
      labs(subtitle = 
             "Red: 3 * mean(hat) for N < 100;  Yellow: 2 * mean(hat) for N >= 100")
}
```
