...
Let's create a function to help us create and combine these plots.

```{r}
#| code-fold: true

plot_univariate <- function(data, var_name, n_bins = 10) {
  data |> 
...
As always, lets by DRY by writing functions!
```{r}
#| code-fold: true
...
plot_dfbetas <- function(model, parameter){
...
rownames(d) <- data$subid

car::avPlots(m, intercept = TRUE,
             id = list(method = list(abs(residuals(m, type = "pearson"))), 
