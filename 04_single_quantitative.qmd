...
:::{.fragment}
[Under the $H_0: \beta_1 = 0$, the sampling distribution for $\beta_1$ will have a mean of 0 with an estimated standard deviation `r round(subset(broom::tidy(m_2), term == "bac")$std.error, 2)`]{style="color:blue;"}
...
[Our value of the parameter estimate, $b_1$, is `r abs(round(subset(broom::tidy(m_2), term == "bac")$estimate/subset(broom::tidy(m_2), term == "bac")$std.error, 2))` standard deviations below the expected mean of the sampling distribution for $H_0$.]{style="color:blue;"}
...
- [You can also compare $b$ to any other non-zero value as well]{style="color:blue;"}
:::
...
```{r}
round(cor.test(data$bac, data$fps)$estimate)^2)
```
...
You are predicting the mean $Y$ for any $X$. 
...
## Predicted means wih 95% Confidence Intervals
...
- We need value for BAC for which to make predictions.  We can put them into an dataframe (see `x` below)
```{r}
