...
<span style="color: red;">Describe the logic of how the t statistic and p-value were determined given your understanding of sampling distributions.</span>
...
<span style="color: blue;">4. We can use `pt()` to calculate the $p$ value.</span>
...
<span style="color: blue;">5. The probability of obtaining a sample $b_0$ = `r round(broom::tidy(m)$estimate, 1)` (or more extreme) if $H_0$ is true is very low (< .05). Therefore we reject $H_0$ and conclude that $\beta_0 \neq 0$ and $b_0$ is our best (unbiased) estimate of it.</span>
