...
:::{.fragment}
[We want to predict some characteristic about the population of FPS scores that minimizes the ERROR from our model.]{style="color:blue;"}

[ERROR = DATA – MODEL]{style="color:blue;"}

[$\varepsilon_i=Y_i-\hat{Y}_i$]{style="color:blue;"}

[There is an error ($\varepsilon_i$) for each population score.]{style="color:blue;"}
:::
...
[1.  **Sum of errors** across all scores in the population isn’t ideal because positive and negative errors will tend to cancel each other out. We won't use this.]{style="color:blue;"}

- [$\sum(Y_i-\hat{Y}_i)$]{style="color:blue;"}


[2.  **Sum of absolute values of errors** could work. If we selected $\beta_0$ to minimize the sum of the absolute value of errors, $\beta_0$ would equal the median of the population.  We could use this, but there is a better option.]{style="color:blue;"}

- [$\sum(|Y_i-\hat{Y}_i|)$]{style="color:blue;"}


[3.  **Sum of squared errors (SSE)** could work. If we selected $\beta_0$ to minimize the sum of squared errors, $\beta_0$ would equal the mean of the population.  This is the best option and what we will use.]{style="color:blue;"}

- [$\sum(Y_i-\hat{Y}_i)^2$]{style="color:blue;"}
:::
...
:::{.fragment}
[We don't know the population mean for FPS scores ($\mu$).]{style="color:blue;"}
...
::::{.fragment}
[We can collect a sample from the population and use the sample mean ($\overline{X}$) as an estimate of the population mean ($\mu$).]{style="color:blue;"}

[$\overline{X}$ is an unbiased estimate for $\mu$.]{style="color:blue;"}
...
1. We will frequently use the `tidy()` function from the broom package so I am loading that  package here.  The alternative is to not load the package but use `broom::tidy()` each time we want to use the function. I prefer to load the package given that its part of the extended tidyverse.
2. My preferred theme for ggplots.  Set it here to affect all plots in this document.
...
1. Create a simple model of FPS scores in the population

- $\text{FPS}=\beta_0$   

2. Collect sample of $N=96$ to estimate $\beta_0$

3. Calculate sample parameter estimate ($b_0$) that minimizes SSE in sample

4. Use $b_0$ to test hypotheses
...
1. Here we are fitting a linear model with FPS regressed on the intercept. In other words, we are fitting a model that predicts FPS using only the sample mean.    
...
1. Again, remember you could have instead used `broom::tidy()` if you didn't load the broom package.
...
:::{.fragment}
[1. Establish null and alternative hypotheses.]{style="color:blue;"}

- [$H_0: \beta_0 = 0; H_a: \beta_0 \neq 0$]{style="color:blue;"}


[2. If $H_0$ is true, the sampling distribution for $b_0$ will have a mean of 0. We can estimate standard deviation of the sampling distribution with SE for $b_0$.]{style="color:blue;"}


[3. $b_0$ is approximately `r round(tidy(m)$estimate/tidy(m)$std.error)` standard deviations above the expected mean of the distribution if $H_0$ is true.]{style="color:blue;"}

- [$t(df=N-P)=\frac{b_0-0}{\text{SE}_{b_0}}=\frac{32.2-0}{3.8}=8.40$]{style="color:blue;"}

[4. We can use `pt()` to calculate the exact $p$ value for this parameter estimate given $H_0$.]{style="color:blue;"}

```{r}
pt(8.40,95,lower.tail=FALSE) * 2
```

[The probability of obtaining a sample $b_0$ = `r round(tidy(m)$estimate, 1)` (or more extreme) if $H_0$ is true is very low (< .05). Therefore we reject $H_0$ and conclude that $\beta_0 \neq 0$ and $b_0$ is our best (unbiased) estimate of it.]{style="color:blue;"}

[Of course, we don't need to calculate the p-value with `pt()` because we got it directly from `tidy()`]{style="color:blue;"}
:::
...
We can calculate the SSE for the augmented model ($\text{SSE}_a = \sum(Y_i- `r round(tidy(m)$estimate, 1)` )^2$) directly:
...
:::{.fragment}
[$\text{CI}(b_0)= b_0 \pm t(\alpha;N-P) * \text{SE}_{b_0}$]{style="color:blue;"}

[For the 95% confidence interval this is approximately 2 SEs around our unbiased estimate of $\beta_0$.]{style="color:blue;"}
...
::::{.fragment}
[If a parameter estimate $\neq$ 0 at $\alpha$ = .05, then the 95% confidence interval for its parameter estimate should not include 0.]{style="color:blue;"}

[This is also true for testing whether the parameter estimate is equal to any other non-zero value for the population parameter.]{style="color:blue;"}
::::
:::
...
:::{.fragment}
[The one sample t-test testing if a population mean = 0.]{style="color:blue;"}
...
:::{.fragment}
[**Option 1:** Compare SSE for the augmented model ($\hat{Y}_i= \beta_0$) to SSE from a different compact model for this new $H_0$ ($\hat{Y}_i= 10$).]{style="color:blue;"}

\

[**Option 2:** Recalculate t-statistic using this new $H_0$.]{style="color:blue;"}

- [$t= \frac{b_0 - 10}{\text{SE}_{b_0}}$]{style="color:blue;"}

\

[**Option 3:** Does the confidence interval for the parameter estimate contain this other value? No p-value provided.]{style="color:blue;"}
